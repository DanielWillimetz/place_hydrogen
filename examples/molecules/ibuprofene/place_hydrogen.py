...
### Filenames
input_filename = 'ibuprofene_no_H.xyz'                     
input_format = 'xyz'  
output_filename = 'ibuprofene_H.xyz'     
output_format = 'xyz'
...
N_charged = False                # Charged N group (adds extra hydrogen on N atom) - either True/False or list of N atoms

# Forcing double/triple/aromatic bonds
single = []                     # Force some bonds to be single - e.g. force single bonds between 1 and 2; 3 and 4 will be [[1,2],[3,4]]
double = []
triple = []
aromatic = []
...
def water_hydrogen(atom_index):
    ...
    hydrogen2 = rotate_point(OH, O, axis, 0.58)
    
    hydrogen = [hydrogen1,hydrogen2]
    return hydrogen

def oxygen_one_hydrogen(atom_index,cutoff):
    nei_cn,vectors = get_neighbors(atom_index, cutoff)
    O = structure[atom_index].position
    A = normalize(vectors[nei_cn[0]]) + O
    arbitrary_vector = np.array([1,0,0])
    axis = normalize(np.cross(O-A, arbitrary_vector))
    OH = normalize(O - A)
    hydrogen1 = rotate_point(OH, O, axis, 1.3)
...
def hydrogen_positions(n_hydr,n_neigh,atom_index,cutoff):
    ...
    if n_neigh == 0 and n_hydr == 2 and structure[atom_index].symbol == "O":
        hydrogens = water_hydrogen(atom_index)
    if n_neigh == 1 and n_hydr == 1 and structure[atom_index].symbol == "O":
        hydrogens = oxygen_one_hydrogen(atom_index,cutoff)
